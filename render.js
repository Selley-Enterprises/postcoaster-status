...
// ── Subscribe form (#991) ────────────────────────────────────────────────────
// Posts to the APP's public endpoint (this page is static, on another host).
// If the app is unreachable the page itself still renders — only signing up is
// unavailable, which we say plainly rather than failing silently.
const APP_ORIGIN = 'https://app.postcoaster.com';

function wireSubscribeForm() {
  const form = $('pcs-subscribe-form');
  const input = $('pcs-subscribe-email');
  const button = $('pcs-subscribe-submit');
  const note = $('pcs-subscribe-note');
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!email || !input.checkValidity()) {
      say('Enter a valid email address.', 'bad');
      input.focus();
      return;
    }
    ...
    try {
      const res = await fetch(`${APP_ORIGIN}/api/status/subscribe`, {
        method: 'POST',
      ...
      if (res.ok) {
        form.hidden = true;
        say(payload.userMessage || 'Check your inbox to confirm the subscription.', 'ok');
      } else {
        say(payload.userMessage || 'We could not sign you up just now. Please try again shortly.', 'bad');
      }
    } catch {
      say('We could not reach PostCoaster to sign you up. Please try again shortly.', 'bad');
    } finally {
